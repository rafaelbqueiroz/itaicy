...
// Page Templates for dynamic header/layout control
export const pageTemplates = pgTable("page_templates", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  slug: text("slug").notNull().unique(),
  headerType: text("header_type").notNull().default("sticky"), // "transparent" or "sticky"
  hasHeroImage: boolean("has_hero_image").notNull().default(false),
...
// Dynamic Suites for CMS management
export const cmsSuites = pgTable("cms_suites", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
...
export const insertCmsSuiteSchema = createInsertSchema(cmsSuites).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type CmsSuite = typeof cmsSuites.$inferSelect;
export type InsertCmsSuite = z.infer<typeof insertCmsSuiteSchema>;
